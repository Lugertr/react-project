...
export function createReducerManager(initialReducers: ReducersMapObject<StateSchema>): ReducerManager {
    ...
    const mountedReducers: MountedReducers = {} as MountedReducers;

    // Инициализируем mountedReducers
    Object.keys(reducers).forEach((key) => {
        mountedReducers[key as StateSchemaKey] = true;
    });
    ...
    return {
        ...
        reduce: (state: StateSchema | undefined, action: UnknownAction): StateSchema => {
            ...
            return combinedReducer(state as object, action);
        },
